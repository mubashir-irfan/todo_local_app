...
const TodoForm: React.FC<TodoFormProps> = ({ onAddTodo, onClose }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const errorMessage = onAddTodo({ name: text.trim(), description: description.trim() });
  ...
  return (
    <form onSubmit={handleSubmit} className="mb-4 min-w-[500px] flex flex-col gap-2">
      <div>
        <input
          type="text"
          value={text}
      ...
      <div>
        ...
        />
        {descriptionError && <p className="text-red-500 text-sm my-1">{descriptionError}</p>}
      </div>
      <div className='flex justify-between items-center'>
        <Button label='Add Todo' type='submit' onClick={() => { }} className='mx-auto' />
      </div>
    </form >
  );
};
